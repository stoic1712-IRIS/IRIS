...
- **Desktop control** is governed by ADR-008 only through an exact application/window target, coordinate-free bounded action schema, metadata-only preview digest, one-shot exact Founder approval, immediate interruption, hard deadline, bounded recovery, replay denial, and mandatory hash-chained audit. It is disabled by default, accepts no secrets, and has no live runner in Cycle Eleven A. Live application or window control remains prohibited until a separately reviewed activation binds the exact merged Core contract.
