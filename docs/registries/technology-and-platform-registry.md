...
| Ephemeral screenshot capture | Playwright-backed, byte- and dimension-bounded, redaction-attested, ephemeral, metadata-only capture behind an IRIS-owned contract | **Proposed contract, not activated**: no live capture, no persistence, no bytes to logs/evidence/model context | ADR-007; Cycle Ten C specification and evidence |
| Credential references | Windows Credential Manager, opaque `wcm://` reference only; IRIS never holds a value | **Proposed contract, not activated**: no enumeration, no resolution, later exact Founder authorization required | ADR-007; Cycle Ten C specification and evidence |
| Local notifications | Windows native local notifications; bounded redacted plain text, non-networked, non-actionable | **Proposed contract, not activated**: local-only, no links/actions/input, no remote delivery, no persistence | ADR-007; Cycle Ten C specification and evidence |
| Desktop control | None | **Blocked**: keyboard/pointer injection, application/window control, and accessibility automation are not adopted, pending a separate Founder-approved ADR covering preview, exact scope, interruption, audit, recovery, and fail-closed behavior | ADR-007 (explicit non-adoption) |
