...
| Playwright | Governed browser inspection and future ephemeral screenshot capture | Ephemeral headless Chromium behind the Cycle Six tool gateway | 1.62.0 | Apache-2.0 | Adopted behind the governed gateway; downloads, service workers, persisted state, and inherited permissions disabled; screenshot capture is a proposed, not-yet-activated contract | Preserve pin and notices; review before any live screenshot activation (ADR-007) |
...
| Windows Credential Manager | Opaque credential reference store | Founder-controlled OS store; IRIS holds only a `wcm://` reference, never a value | Windows 11 OS component | Proprietary OS terms; use only | Proposed reference-only contract, not activated; no enumeration, no resolution without a later exact Founder authorization | Record exact resolution authorization design before any activation (ADR-007) |
| Windows native notifications | Local Founder signal | Local-only, non-networked, bounded redacted plain-text notification | Windows 11 OS component | Proprietary OS terms; use only | Proposed local-only contract, not activated; no links, actions, input, remote destination, or persistence | Record delivery-adapter design before any activation (ADR-007) |
