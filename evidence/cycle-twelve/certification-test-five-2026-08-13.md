...
- Open and unrecorded elsewhere: in the canonical `C:\Projects\STOIC-IRIS` checkout, Node cannot resolve packages through the pnpm `node_modules` symlinks (`EACCES` on `node_modules/zod/package.json`), so `pnpm verify` and `node scripts/dev/iris-dev.mjs contract inspect` cannot run locally there. CI remains the authority for this record. Repairing it requires a dependency operation this task does not authorize.
